...
def students(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'create':
            ...
            put_response = requests.put(f'{API_BASE_URL}/students/{request.POST["id"]}', json={
                'name': request.POST['name'],
                'email': request.POST['email']
            }, headers={'Authorization': f'Bearer {token}'})
            if put_response.status_code != 200:
                messages.error(request, f"Failed to update student: {put_response.text}")
        elif action == 'delete':
            delete_response = requests.delete(f'{API_BASE_URL}/students/{request.POST["id"]}', headers={'Authorization': f'Bearer {token}'})
            if delete_response.status_code != 200:
                messages.error(request, f"Failed to delete student: {delete_response.text}")
...
def bookings(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'create':
            ...
            put_response = requests.put(f'{API_BASE_URL}/bookings/{request.POST["id"]}', json={
                'student_id': request.POST['student_id'],
                'room_id': request.POST['room_id']
            }, headers={'Authorization': f'Bearer {token}'})
            if put_response.status_code != 200:
                messages.error(request, f"Failed to update booking: {put_response.text}")
        elif action == 'delete':
            delete_response = requests.delete(f'{API_BASE_URL}/bookings/{request.POST["id"]}', headers={'Authorization': f'Bearer {token}'})
            if delete_response.status_code != 200:
                messages.error(request, f"Failed to delete booking: {delete_response.text}")
